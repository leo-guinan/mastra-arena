...
const observeStream = createTool({
  id: "observe-stream",
...
const listCrons = createTool({
  ...
  execute: async ({ agentId }) => ({
    jobs: Array.from(jobs.values()).filter((j) => j.agentId === agentId).map((j) => ({
      ...
      name: j.name,
      status: j.status,
      intervalMinutes: j.intervalMs / 6e4,
      runCount: j.runCount,
...
const agents$1 = /* @__PURE__ */ new Map();
function getOrCreate(agentId) {
  if (!agents$1.has(agentId)) {
    agents$1.set(agentId, {
      agentId,
      intervalMs: 5 * 60 * 1e3,
      lastBeat: null,
      dag: [],
      inbox: [],
      status: "green",
      metrics: { tokenVolume24h: 0, contentProduced: 0, audienceEngagement: 0, revenue: 0 }
    });
  }
  return agents$1.get(agentId);
}
const heartbeatTool = createTool({
  id: "heartbeat",
  ...
  inputSchema: z.object({
    ...
    metrics: z.object({
      tokenVolume24h: z.number().default(0),
      contentProduced: z.number().default(0),
      audienceEngagement: z.number().default(0),
  ...
  execute: async ({ agentId, completed, inProgress, queued, blocked, status, metrics }) => {
    ...
    for (const t of inProgress) {
      const ex = state.dag.find((d) => d.id === t.id);
      if (ex) ex.status = "in-progress";
      else state.dag.push({ id: t.id, name: t.name, status: "in-progress", dependsOn: [] });
    }
    for (const t of queued) {
      const ex = state.dag.find((d) => d.id === t.id);
      if (ex) {
        ex.status = "queued";
        ex.dependsOn = t.dependsOn;
...
const getHeartbeatStatus = createTool({
  ...
  execute: async ({ agentId }) => {
    const targets = agentId ? [getOrCreate(agentId)] : Array.from(agents$1.values());
    return {
      agents: targets.map((a) => ({
        agentId: a.agentId,
        status: a.status,
        lastBeat: a.lastBeat,
        secondsSinceLastBeat: a.lastBeat ? Math.round((Date.now() - a.lastBeat) / 1e3) : null,
...
const getTokenPrice = createTool({
  id: "get-token-price",
...
const getMultipleTokenPrices = createTool({
...
async function farcasterRequest(endpoint, method, body) {
...
const postCast = createTool({
  ...
  execute: async ({ text, parentHash, parentUrl }) => {
    ...
    const data = await farcasterRequest("/cast", "POST", body);
    return { hash: data.hash, message: "Cast posted" };
  }
});
const postThread = createTool({
  id: "post-thread",
...
const getNotifications = createTool({
  ...
  execute: async ({ limit }) => {
    const data = await farcasterRequest(`/notifications?limit=${limit}`, "GET");
    return {
...
const searchCasts = createTool({
  ...
  execute: async ({ query, limit }) => {
    const data = await farcasterRequest(`/search?q=${encodeURIComponent(query)}&limit=${limit}`, "GET");
    return {
...
const agents = { skippy, mando, walle, docBrown, marvin };
...
const collectHeartbeats = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    return { heartbeats, timestamp: (/* @__PURE__ */ new Date()).toISOString() };
  }
});
const computeRankings = createStep({
  ...
  execute: async ({ inputData }) => {
    const scored = inputData.heartbeats.map((hb) => {
      const score = hb.tokenVolume24h * 0.4 + hb.contentProduced * 100 * 0.2 + hb.audienceEngagement * 50 * 0.2 + hb.revenue * 0.2;
      return {
        agentId: hb.agentId,
...
const arenaHeartbeat = createWorkflow({
  id: "arena-heartbeat",
  inputSchema: z.object({
...
}).then(collectHeartbeats).then(computeRankings).then(generateCommentary).commit();
